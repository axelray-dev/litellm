...
class SAMLAuthHandler:
    ...
    async def _load_idp_settings(cache: DualCache) -> dict[str, object]:
        ...
        idp_settings = cast(dict[str, object], parsed)
    ...
    async def build_login_redirect(
        ...
        redirect_url = cast(str, auth.login(return_to=relay_state))
        response = RedirectResponse(url=redirect_url, status_code=303)
        request_id = cast(Optional[str], auth.get_last_request_id())
    ...
    async def build_sp_metadata(request: Request, cache: DualCache) -> str:
        ...
        metadata = cast(str, saml_settings.get_sp_metadata())
        errors = cast(list[str], saml_settings.validate_metadata(metadata))
        if errors:
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    async def handle_acs(request: Request, cache: DualCache, post_data: dict[str, str]) -> CustomOpenID:
        ...
        errors = cast(list[str], auth.get_errors())
    ...
    async def _enforce_response_binding(
        ...
        in_response_to = cast(Optional[str], auth.get_last_response_in_response_to())
        ...
        assertion_id = cast(Optional[str], auth.get_last_assertion_id())
    ...
    def _result_from_auth(auth: "OneLogin_Saml2_Auth") -> CustomOpenID:
        ...
        name_id = cast(Optional[str], auth.get_nameid())
